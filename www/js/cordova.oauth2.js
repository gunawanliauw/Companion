 ...
 * 
 * Usage:
 *   jQuery.oauth2(options, successCallback, errorCallback);
 *
 *   jQuery.oauth2({
 *        auth_url: '',         // required
...
(function(jQuery){            
    jQuery.oauth2 = function (options, successCallback, errorCallback) {
        ...
        jQuery.extend(paramObj, options.other_params);
        var login_url = options.auth_url + '?' + jQuery.param(paramObj);

        // open Cordova inapp-browser with login url
        var loginWindow = window.open(login_url, '_blank', 'location=yes,toolbar=no');

        // check if redirect url has code, access_token or error 
        jQuery(loginWindow).on('loadstart', function(e) {
    ...
    jQuery.oauth2_getAccess_token = function(options, successCallback, errorCallback){
        var checkOauth2Params = function(options){
            var missing = "";
            if(!options.client_id) {missing += " client_id"}
            if(!options.auth_url) {missing += " auth_url"}
